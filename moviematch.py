...
# 3. Premium Studio Styling Engine
st.markdown("""
<style>
    ...
    .title-container { display: flex; align-items: center; gap: 18px; margin-bottom: 0px; }
    .title-emoji { font-size: 3.5rem; }
    .main-title { font-weight: 800; color: #FFFFFF; font-size: 3.5rem; letter-spacing: -1.5px; margin: 0; }
    .sub-title { color: #00F0FF; font-weight: 600; font-size: 0.95rem; letter-spacing: 3px; margin-bottom: 40px; text-transform: uppercase; opacity: 0.9; }
    .welcome-card { background: rgba(255, 255, 255, 0.03); border: 1px solid rgba(255, 255, 255, 0.07); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); padding: 30px; border-radius: 24px; margin-bottom: 40px; box-shadow: 0 20px 50px rgba(0, 0, 0, 0.4); }
    div[data-testid="stVerticalBlockBorderWithBorders"] { background: rgba(255, 255, 255, 0.02) !important; border: 1px solid rgba(255, 255, 255, 0.08) !important; backdrop-filter: blur(25px) !important; -webkit-backdrop-filter: blur(25px) !important; border-radius: 32px !important; padding: 28px !important; transition: all 0.5s cubic-bezier(0.16, 1, 0.3, 1) !important; box-shadow: 0 15px 35px rgba(0, 0, 0, 0.5) !important; }
    div[data-testid="stVerticalBlockBorderWithBorders"]:hover { transform: translateY(-12px) scale(1.02) !important; border-color: rgba(255, 46, 147, 0.4) !important; box-shadow: 0 30px 60px rgba(255, 46, 147, 0.15) !important; }
    div[data-testid="stForm"] { border: none !important; padding: 0 !important; }
    label p { color: #E2E8F0 !important; font-size: 1.1rem !important; font-weight: 600 !important; }
</style>
...
# ==========================================
    ...
    with st.spinner("⚡ Calibrating screening array... parsing digital artwork files..."):
        ...
        for i, movie in enumerate(movie_data.get("movies", [])[:3]):
            title = movie.get("title", "Unknown Title")
            year = movie.get("year", 2026)
            rating = movie.get("rating", 0)
            why_it_fits = movie.get("why_it_fits", "")
            
            poster_url = get_movie_poster(title, year)
            search_query = urllib.parse.quote(f"where to watch {title} {year}")
            google_search_url = f"https://www.google.com/search?q={search_query}"
            
            with columns[i]:
                with st.container(border=True):
                    st.image(poster_url, use_container_width=True)
                    st.markdown(f"<h3 style='margin-top:15px; margin-bottom:5px; font-weight:800; color:#FFF; font-size:1.5rem; letter-spacing:-0.5px;'>{title}</h3>", unsafe_allow_html=True)
                    st.markdown(f"<p style='color:#94A3B8; font-size:0.9rem; margin-bottom:20px;'>🗓️ Year: {year} &nbsp;|&nbsp; <span style='color:#00F0FF; font-weight:600;'>⭐ Match: {rating}/10</span></p>", unsafe_allow_html=True)
